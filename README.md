...
## 一、项目总览
### 1. 项目简介 📖
本项目按照 **State状态 → Node节点 → Edge流程分支** 标准学习路线拆分模块，覆盖 LangGraph 全部核心功能，包含：
基础语法、性能优化、持久化会话、人机交互中断、循环分支、底层Pregel执行原理等完整实战案例。
...
├─ 260616/
│ ├─ 01_state/ # 💾 模块 2：持久化检查点、会话恢复、底层 Pregel 原理
│ │ ├─ sqlite_data/
...
│ └─ 02_nodes/ # ⚙️ 模块 3：节点全特性（缓存 / 重试 / 流式 / 中断交互）
...
覆盖从入门 Demo 到生产持久化、底层 Pregel 原理全链路内容
统一全局模型配置文件，开箱即用，无需重复修改密钥
